 ...
 * @author Uwe Hennig
 */
public class AxonPersistence {
    public static void save(AxonModel model, Path file) throws IOException {
    ...
    public static AxonModel laod(Path file) throws IOException {
        try (FileChannel channel = FileChannel.open(file, READ)) {
            int size = (int) channel.size();
            int capacity = size / (int) AxonModel.LAYOUT.byteSize();
            AxonModel model = new AxonModel(capacity);
